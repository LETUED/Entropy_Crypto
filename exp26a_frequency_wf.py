"""
실험 26A: 진입 빈도 최적화 Walk-forward

기반: Exp23-D (가격MPE<10%ile AND 볼륨MPE>=50%ile, WF Sharpe +1.291, 거래 37건)

목표: RSI<35 + Stochastic %K<20 + Williams %R<-80 조합으로
      거래 빈도 37 → 60+ 건 (통계적 신뢰도 강화)
      품질 기준: WF Sharpe ≥ +1.0, 양수구간 ≥ 4/6

이론 근거:
  arXiv:2410.06935v1 (2024): RSI+Stochastic+Williams 앙상블 → 92.4% 정확도
  MDPI 1911-8074/17/11/501 (2023): Stochastic+Williams%R 조합 → 거짓신호 필터링
  PMC9920669 (2022): RSI 단독 취약, 다중지표 조합 권장

비교 조건 (O5: BTC, SOL, AVAX, ADA, DOT):
  A (기준):  RSI<30                      (Exp23-D 그대로)
  B (확장):  RSI<35                      (임계값만 완화)
  C (앙상블): RSI<35 AND Stochastic%K<20  (빈도↑ + 품질 유지)
  D (유니온): RSI<35 AND (Stochastic%K<20 OR Williams%R<-80) (최대 빈도)
...
VOL_PCT_D   = 50   # 볼륨 MPE >= 50%ile (Exp23-D)

RSI_OVERSOLD_A = 30   # 기준 (Exp23-D)
RSI_OVERSOLD_B = 35   # B/C/D 확장
...
# ── 기술적 지표 (로컬 정의, look-ahead 없음) ──────────────────────────────────
def compute_stochastic(df: pd.DataFrame, k_period: int = 14) -> pd.Series:
    """Stochastic %K — 과거 k_period봉의 고가/저가 범위 내 현재 가격 위치 (0~100)"""
...
def compute_williams_r(df: pd.DataFrame, period: int = 14) -> pd.Series:
    """Williams %R — -100~0 범위. -80 이하 = 과매도 구간."""
...
def _run_window(df_test, price_mpe_test, vol_mpe_test,
                stoch_k_test, williams_r_test, h_test,
    ...
    mode:
      A — Exp23-D 기준 (RSI<30)
      B — RSI<35 (임계값 확장)
      C — RSI<35 AND Stochastic%K<20
      D — RSI<35 AND (Stochastic%K<20 OR Williams%R<-80)
    """
    ...
    for i, idx in enumerate(df_test.index):
        ...
        stoch   = stoch_k_test.loc[idx]   if idx in stoch_k_test.index   else np.nan
        will_r  = williams_r_test.loc[idx] if idx in williams_r_test.index else np.nan
        ...
        if mode == "A":
            ...
            sig_ok = rsi_val < RSI_OVERSOLD_B
        elif mode == "C":
            sig_ok = (rsi_val < RSI_OVERSOLD_B
                      and not np.isnan(stoch) and stoch < 20)
        else:  # D
            sig_ok = (rsi_val < RSI_OVERSOLD_B
                      and ((not np.isnan(stoch) and stoch < 20)
                           or (not np.isnan(will_r) and will_r < -80)))
...
# ── 단일 코인 Walk-forward ────────────────────────────────────────────────────
def run_coin_wf_all(sym, df, price_mpe, vol_mpe, stoch_k, williams_r, h_onchain):
    ...
    while True:
        ...
        will_test   = williams_r[test_start:test_end]
...
# ── 거래 데이터 수집 (Exp26B SHAP용) ─────────────────────────────────────────
def collect_trades_for_shap(coin_results_by_mode, coin_data, mode="C"):
    """
    Exp26B SHAP 분석용: 지정 모드의 모든 거래에서 피처 + 결과 추출
    Returns: DataFrame with columns [피처8개, pnl_pct, label, sym, period]
    """
    records = []
    for sym in COINS:
        for win in coin_results_by_mode[sym][mode]:
            df_period  = coin_data[sym]["df"][win["test_start"]:win["test_end"]]
            price_mpe  = coin_data[sym]["price_mpe"]
            vol_mpe    = coin_data[sym]["vol_mpe"]
            stoch_k    = coin_data[sym]["stoch_k"]
            h_onchain  = coin_data[sym]["h"]
            btc_close  = coin_data["BTCUSDT"]["df"]["close"]

            train_s = win["test_start"] - pd.DateOffset(months=TRAIN_MONTHS)
            train_e = win["test_start"]
            p_train = price_mpe[train_s:train_e].dropna()
            v_train = vol_mpe[train_s:train_e].dropna()
            h_train = h_onchain[train_s:train_e].dropna() if h_onchain is not None else pd.Series(dtype=float)

            for t in win["trades"]:
                # 거래 진입 시점을 찾아 피처 추출
                # trades에는 pnl_pct와 held_h만 있으므로, 피처는 진입 시점에서 계산
                # 여기서는 거래 결과 데이터를 그대로 활용
                pnl = t["pnl_pct"]
                records.append({
                    "sym":           sym,
                    "period":        win["short_lbl"],
                    "pnl_pct":       pnl,
...
def plot_results(all_port):
    ...
    mode_names = {
        "A": "A: RSI<30 (Exp23-D 기준)",
        "B": "B: RSI<35 (확장)",
        "C": "C: RSI<35 AND Stoch%K<20",
        "D": "D: RSI<35 AND (Stoch<20 OR Williams<-80)",
    }
    ...
    ax2.set_title("구간별 포트폴리오 Sharpe (RSI+Stochastic+Williams 조건별)", color="#e6edf3")
    ...
    ax4.axhline(50, color="#56d364", linewidth=1.0, linestyle="--",
                label="목표 50건")
    ax4.set_xticks(range(4))
    ax4.set_xticklabels([mode_names[m] for m in all_port], color="#8b949e", fontsize=7.5)
    ax4.set_title("조건별 총 거래수 (녹색선=목표 50건)", color="#e6edf3")
    ax4.set_ylabel("총 거래수", color="#8b949e")
    ax4.legend(fontsize=8, facecolor="#21262d", edgecolor="#30363d", labelcolor="#e6edf3")

    fig.suptitle(
        "Exp26A: 진입 빈도 최적화 Walk-forward (O5 포트폴리오)\n"
        "A(RSI<30기준) vs B(RSI<35) vs C(RSI<35+Stoch) vs D(RSI<35+Stoch/Williams)",
        color="#e6edf3", fontsize=12, fontweight="bold", y=1.01,
    )

    path = RESULTS_DIR / "exp26a_frequency_wf.png"
...
def main():
    print("=" * 70)
    print("Exp26A: 진입 빈도 최적화 Walk-forward")
    print(f"  코인: {', '.join(LABEL[s] for s in COINS)}")
    print(f"  기간: {START} ~ {END}")
    print("  기준 (A): Exp23-D (RSI<30 + 가격MPE<10% + 볼륨MPE>=50%)")
    print("  조건  B:  RSI<35")
    print("  조건  C:  RSI<35 AND Stochastic%K<20")
    print("  조건  D:  RSI<35 AND (Stochastic%K<20 OR Williams%R<-80)")
    print("=" * 70)
    ...
    for sym in COINS:
        ...
        # Stochastic %K, Williams %R (OHLCV 필요, 캐시 없음 → 즉시 계산)
        stoch_k   = compute_stochastic(df, k_period=14)
        will_r    = compute_williams_r(df, period=14)
        ...
        # Stochastic/Williams 분포 확인
        sk_mean = stoch_k.dropna().mean()
        sk_lt20 = (stoch_k.dropna() < 20).mean() * 100
        wr_mean = will_r.dropna().mean()
        wr_ltm80 = (will_r.dropna() < -80).mean() * 100
        print(f"  [{LABEL[sym]:6s}] {len(df):,}봉 | "
              f"Stoch%K avg={sk_mean:.1f} (<20: {sk_lt20:.1f}%) | "
              f"Williams avg={wr_mean:.1f} (<-80: {wr_ltm80:.1f}%)")
    ...
    print(f"  {'조건':<6} | {'평균 Sharpe':>12} | {'개선폭':>8} | {'양수구간':>10} | {'거래수':>6} | {'목표달성':>8}")
    print("  " + "-" * 62)
    base_s = summary["A"]["avg_sharpe"]
    for mode in ["A", "B", "C", "D"]:
        s        = summary[mode]
        delta    = s["avg_sharpe"] - base_s
        mark     = "★" if s["avg_sharpe"] >= 1.0 and s["total_n"] >= 50 else " "
        achieved = "✅" if s["avg_sharpe"] >= 1.0 and s["total_n"] >= 50 else "  "
        print(f"  {mode}{mark}     | {s['avg_sharpe']:>+12.3f} | {delta:>+8.3f} | "
              f"{s['pos_cnt']}/6{' ':>7} | {s['total_n']:>6} | {achieved:>8}")
    print("=" * 70)
    print("  목표 달성 기준: Sharpe ≥ +1.0 AND 거래수 ≥ 50")

    # Exp26B용 최적 모드 저장 (pickle 없이 CSV로)
    best_mode = max(summary, key=lambda m: (
        1 if summary[m]["total_n"] >= 50 else 0,
        summary[m]["avg_sharpe"]
    ))
    print(f"\n  → Exp26B (SHAP 분석) 권장 기준 모드: {best_mode} "
          f"(Sharpe={summary[best_mode]['avg_sharpe']:+.3f}, "
          f"거래={summary[best_mode]['total_n']}건)")

    print("\n[시각화 생성...]")
    plot_results(all_port)
    print("\nExp26A 완료!")

    return coin_results_by_mode, coin_data, summary, best_mode
